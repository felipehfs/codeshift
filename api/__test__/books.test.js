...
async function createBook() {
    const creationRequest = await supertest(app).post('/api/books')
    .send({ 
            name: 'teste$56',
...
describe("App", () => {
    ...
    test('should register the user', async () => {
        const response = await supertest(app).post(`/api/register`)
            .send({ email: 'felipe@gmail.com', password: '12345678', username: 'felipe'});
    ...
    test('should remove a book', async () => {
        ...
        const response = await supertest(app).delete(`/api/books/${newBook._id}`)
            .set('Authorization', `Bearer ${token}`);
